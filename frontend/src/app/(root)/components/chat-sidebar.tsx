                ...
                <div className={`flex items-center ${collapsed ? "justify-center" : ""}`}>
                    ...
                    {!collapsed && (
                        <div className="flex items-center">
                            <div className="w-7 h-7 mr-3 bg-[#3a1a2f] rounded-lg flex items-center justify-center">
                        ...
                        {Object.keys(groupedConversations).length === 0 ? (
                            ...
                            Object.entries(groupedConversations).map(([date, conversations]) => (
                                <div key={date} className="mb-1">
                                    <div className="px-4 py-3 text-xs font-medium text-gray-400 uppercase tracking-wider">{date}</div>
                                    {conversations.map((conversation) => (
                                        <div
                                            key={conversation.id}
                                            onClick={() => onChatSelect(conversation.id)}
                                            className={`mx-2 mb-1 px-3 py-3 rounded-xl cursor-pointer text-gray-300 text-sm transition-colors duration-150 hover:bg-[#2a2a2a] relative ${activeChat === conversation.id ? "bg-[#2a1a2f]" : ""
                                                }`}
                                        >
                                            ...
                                            <div className="flex flex-col">
                                                ...
                                                <div className="text-xs text-gray-400 line-clamp-1 mb-1">{conversation.preview}</div>
                    ...
                    <div className="flex flex-col items-center gap-2 p-2">
                        {filteredConversations.slice(0, 8).map((conversation) => (
                            <button
                                key={conversation.id}
                                onClick={() => onChatSelect(conversation.id)}
                                className={`w-12 h-12 rounded-xl flex items-center justify-center text-xs font-bold transition-colors duration-150 ${activeChat === conversation.id
